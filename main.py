...
# ========================================
@bot.tree.command(name="admin_reset_waiting", description="全ての待機・対戦リストを初期化します（管理者専用）")
async def admin_reset_waiting(interaction: discord.Interaction):
    if interaction.user.id != ADMIN_ID:
        await interaction.response.send_message("このコマンドは管理者専用です。", ephemeral=True)
        return

    # グローバル変数として宣言（既存定義を上書きしないように）
    global waiting_users, in_match, pending_reports

    # 存在するリストだけ初期化（KeyError対策つき）
    try:
        if "waiting_users" in globals():
            waiting_users.clear()
        if "in_match" in globals():
            in_match.clear()
        if "pending_reports" in globals():
            pending_reports.clear()
    except Exception as e:
        await interaction.response.send_message(f"⚠️ リセット中にエラーが発生しました: {e}", ephemeral=True)
        return
